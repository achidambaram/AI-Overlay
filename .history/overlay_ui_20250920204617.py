...
class OverlayUI:
    ...
    def update_suggestions(self, suggestions: List[Dict]):
        """Update suggestions in the modern overlay"""
        if self.is_visible and hasattr(self, 'scrollable_frame'):
            self._add_modern_suggestions(suggestions)
            self._show_status(f"Updated with {len(suggestions)} suggestions")
    
    def hide_overlay(self):
        """Hide the overlay with modern fade animation"""
        if self.is_visible:
            self._start_modern_fade_animation(-1)
    
    def show_overlay(self):
        """Show the overlay with modern fade animation"""
